...
// No libyuv!
// avifImageRGBToYUVLibYUV removed - encoding function not needed for decoding
...
//      char* version_string = (char*)malloc(512);
//      snprintf(version_string, sizeof(version_string), "libavif: %s. Codecs: %s.%s",
//               avifVersion(), codec_versions, libyuv_version);
//      return version_string;
...
avifBool isAvifImage(uint8_t *address, int length){
    avifROData avif = {address, (size_t)length};
    return avifPeekCompatibleFileType(&avif);
}


  InfoCpp *getInfo(uint8_t *address, int length){
     AvifDecoderWrapper decoder;

     if (!CreateDecoderAndParse(&decoder, address, length, /*threads=*/1)) {
        return false;
     }
    //OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, "avif", "----- CreateDecoderAndParse true");
...
void destroyDecoder(int64_t decoder){
    ...
    if (decoderWrapper->decoder != NULL) {
        avifDecoderDestroy(decoderWrapper->decoder);
    }
    free(decoderWrapper);
}


int32_t nextFrameIndex(int64_t decoder){
    AvifDecoderWrapper* const decoderWrapper = (AvifDecoderWrapper*)decoder;
    return decoderWrapper->decoder->imageIndex + 1;
}
...
int32_t nthFrame(int64_t decoder, uint32_t n,uint8_t *pixels,uint32_t picwidth,uint32_t picheight,uint32_t stride,uint32_t formatvalue){
    AvifDecoderWrapper* const decoderWrapper = (AvifDecoderWrapper*)decoder;
    return DecodeNthImage(decoderWrapper, n, pixels,picwidth,picheight,stride,formatvalue);
}
...
bool decode(uint8_t *address,int length,uint8_t *pixels,uint32_t picwidth,uint32_t picheight,uint32_t stride,uint32_t formatvalue, int threads){
      ...
      result=  DecodeNextImage(&decoder,pixels,picwidth,picheight,stride,formatvalue) == AVIF_RESULT_OK;
      avifDecoderDestroy(decoder.decoder);
      decoder.decoder = NULL;
      return result;
}

//FUNC(jboolean, decode, jobject encoded, int length, jobject bitmap,jint threads) {
//  IGNORE_UNUSED_JNI_PARAMETERS;
//  const uint8_t* const buffer =static_cast<const uint8_t*>(env->GetDirectBufferAddress(encoded));
//  AvifDecoderWrapper decoder;
